...
def encode_data_with_pretrained(data_source, train_ds, test_ds, text_field, encoder_model, examples_l, examples_u):
    ...
    if not embeddings_file.is_file():
        create_pretrained_embeddings(train_ds, test_ds, text_field, data_source, embedding_type)
        embeddings_file = Path(data_source_embeddings_path)
...
def create_pretrained_embeddings(train_ds, test_ds, text_field, data_source, embedding_type):
    if embedding_type == "glove":
        encoder = create_encoder(text_field.vocab, 300, "pool_max", *['max'])
        encoder.eval()
        embeddings = {}
        for ds in (train_ds, test_ds):
            sents = [torch.tensor([[text_field.vocab.stoi[t] for t in eg.x]]) for eg in ds.examples]
            for eg, idxed in zip(ds.examples, sents):
                embeddings[' '.join(eg.x)] = encoder(idxed.reshape(-1, 1)).detach().squeeze(0).numpy()
        ...
        embeddings = {}
        for ds in (train_ds, test_ds):
            sents = [torch.tensor([[vocab.stoi[t] for t in eg.x]]).reshape(-1,1) for eg in ds.examples]
            for eg, idxed in zip(ds.examples, sents):
                embeddings[' '.join(eg.x)] = encoder(idxed).detach().squeeze(0).numpy()
        ...
        embeddings = {}
        for ds in (train_ds, test_ds):
            for i, eg in enumerate(ds.examples):
                sent = ' '.join(eg.x)
                S = Sentence(sent)
                encoder.embed(S)
                embeddings[sent] = torch.max(torch.stack([t.embedding for t in S]), 0)[0].detach().numpy()
